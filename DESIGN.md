...
## 足読み gen2 — stocks_minute 歴史分足での再学習（2026-07-16 追加）

owner 指示「`S:/jp/stocks_minute` を使って再学習。銘柄別 / 相関銘柄インプット /
複数銘柄同時学習+シグナル銘柄エントリー等を試行錯誤」。**新 family
`gen2_minute_v1`**（honest-N +1）。実装は `src/scalp_agent_bars/minute/`、
パイプラインは `scripts/gen2_minute_pipeline.py`。

- **データ**: 銘柄別 duckdb の 1 分足 OHLCV（J-Quants 系・Time はバー開始分）。
  フル履歴（2024-01-04〜2026-02-18・519 日）を持つ **17 銘柄に事前固定**
  （他は 2024-04〜2025-03 のみ・5016 は 13 日で不可）。
- **日割り（凍結）**: train 2024-01-04〜2025-06-30 / val 2025-07-01〜09-30 /
  **OOS 2025-10-01〜2026-02-18 封印**。G4 の D≥20 を初めて正式に満たせる構成。
- **執行（保守的バー執行モデル・板なし）**: 決定=バー確定時刻、エントリ=**次バー
  始値+不利半スプレッド**（次バーが 1 分後に無い決定は無効 — 昼休み・欠損跨ぎ禁止）。
  同一バー内で TP/SL 両接触は **SL 扱い**、ギャップは SL=始値（不利）/ TP=バリア価格
  （有利は放棄）。timeout=エントリ含め H 本保有後の次バー始値、14:55 強制決済優先。
  exit にも不利半スプレッド。gross−friction=net の恒等式は gen1 と同一関数で検算。
- **friction（凍結較正）**: 板録画 07-09+07-13 の実測 median spread_bps（銘柄別
  1.5〜5.3bps）× 安全係数 1.25。07-14 は gen1/gen1b の封印日のため**使わない**。
  2026-07 の実測を 2024〜25 年に適用する近似は係数と本節で明示。
- **バリア**: スプレッド錨は分足で解像できないため **ATR20 錨**（Δ=mult×ATR）。
- **凍結格子**: horizon_bars {5,15,30} × atr_mult {1.0,2.0} × τ {0.45,0.55,0.65}
  = 18 セル/パターン。LightGBM は gen1 と同一固定ハイパラ。
- **3 パターン（owner 指示の試行錯誤・事前宣言）**:
  P1_single=銘柄別モデル（自銘柄 19 特徴）/ P2_cross_features=プール学習+クロス
  特徴 26 個（leave-self-out 市場平均リターン・ブレス・市場相対・train 窓日次相関で
  凍結した最相関ペアのリターン）/ P3_pooled_topk=P2 モデル+各分境界で score 最大の
  1 銘柄のみエントリー。
- **val 候補条件**: n≥300 ∧ D≥20 ∧ net/entry>0 ∧ gross/friction≥3 ∧
  単一銘柄 net シェア≤0.5。候補ゼロなら IS-KILL（OOS 封印維持）。候補があれば
  (pattern, cell, τ) を 1 つ凍結し、train+val 再学習で OOS へ 1 回だけ適用
  （`oos_lock.json` が再実行を拒否）。val を見た後の構成変更は次の新 family。
- config hash `7f05c680…`・feature schema hash `f2fef144…`（テストで固定）。

## 足読み gen3 — 系列 NN による学習器対照実験（2026-07-16 追加・IS-KILL）

gen2 IS-KILL への owner の疑問「学習が弱いから gross ゼロでは？」に答える対照実験
（owner 指示）。**新 family `gen3_minute_nn_v1`**。実装は
`src/scalp_agent_bars/minute/{nn_config,sequences,nn_model}.py` +
`scripts/gen3_minute_nn_pipeline.py`、依存は pyproject `nn` グループ（torch cu124）。

- **変えたのは学習器と入力表現のみ**: 手作り 26 特徴 → 当日直近 32 本の生分足系列
  （8ch）+ 静的 4 特徴、LightGBM → GRU 2 層 + 6 セル・マルチタスクヘッド。
  ラベル・保守的バー執行・friction・分割・候補条件は gen2 と行単位で同一
  （gen2 キャッシュとの b_ts 完全一致を assert）。
- **学習規律**: fit=2024-01〜2025-04 / early stop は train 末尾窓 2025-05〜06 のみ
  （公式 val は掃引まで不使用）。OOS は再学習なしで凍結モデルを適用する設計。
- **結果**: val 62 日・n≥300 の全 10 構成で gross 負（中央値 −0.18bps・最大 −0.05bps）
  → **IS-KILL**（台帳 376 行目）。NN は τ≥0.55 でほぼ発火せず、確信を持てる局面
  自体が存在しないと回答。「学習器の弱さ」仮説は棄却。sealed OOS
  （2025-10-01〜2026-02-18）は未開封のまま次の新 family に使える。
