...
- **§3/§6 採点規律 — literal PASS を額面で受けず mechanical artifact を疑う（2026-06-01 #17 peer-relative-dislocation Phase 0 で実証）**: pre-registered gate が literal に PASS でも、その PASS が**測定定義由来の機械的 artifact**なら Adopt にしてはいけない（司令塔の §3 統合・§6 採点の核）。本 issue で実際に踏んだ 2 つの type:
  ...
  - **(vii) §3/§6 採点 — too-good な sweep/最適化結果は「どのコストレバーが殺すか」を予測せず、集中度監査を第一の反証手段にする（conviction-pyramid Phase 0g 損切バリアントで実証・2026-06-04・司令塔 prediction-error）**: no-edge と分かっている line で、パラメータ sweep（損切 hard%/trail% 等）の一部 config が net-positive・PF>1 と出たとき、**そのプラスがどの理想化（cap 省略 / impact 省略 / 無制限ピラミッド）由来かを予測して "その1つを直せば崩れる" と賭けてはいけない**。本件: 緩い損切 config の +2.7M を「無制限ピラミッド（peak 40単位）が原因、notional cap（1%ADV/10%バー出来高）＋sqrt impact を入れれば崩壊する」と予測 → **外れ**（cap で平均 peak は 40→8 に縮んだが net は +2.7M 生存、cap が他の負け add を弾いて改善した config すらあった）。**決定打は cap でなく集中度監査**: net の **93.9% が1銘柄・104.8% が1日（出来高爆発日に cap が 44単位ピラミッドを許した1トレード）**、top-1 除外で ≈breakeven・top-5 除外で −1.17M。対処の型: (a) **「too good」を見たら最初に集中度監査**（利益の top-name / top-month / top-day シェア、net excluding top-k、銘柄ベースの win/loss 数）を回す — これが single-name/single-day outlier（Gate A rev_top1=81% と同型）を最速で暴く。(b) **notional cap は単発テールを消せない**（出来高爆発日は cap 自体が持ち上がり、まさにその日に巨大玉を許す）。(c) コストレバー（cap/impact/fee）の追加は補助的検証で、artifact の有無を決めるのは集中度・per-day 分布。これは risk-manager の集中度/capacity ゲートを「too-good 結果には反射的に回す」運用と同義（本件は司令塔が inline で実施）。CLAUDE.md「見かけの数字を現象と即断しない」の sweep-optimization 版＝**最適化が拾った正値は、まず『何が担っているか』を数えてから信じる**。auto-memory [[intraday-trend-continuation-selection-no-edge]]。
...
> **§6 落とし穴 — `edit_note(operation='append')` の content 先頭に `---` 区切り（水平線）を置かない（2026-06-04 conviction-pyramid Phase 0g 実証）**: append content の先頭行が `---` だと BM が **frontmatter document separator と誤認**して `Invalid YAML in frontmatter: expected '<document start>'` で失敗する（dated section 間を `---` で区切る proposals では起きやすい）。対処: append content は `## <見出し>` から始める（先頭 `---` を外す）。物理ファイル内の section 区切り `---` が要るなら、append が成功した後に物理 Edit で足すか、`---` を見出しの後ろに置く。あわせて **§6 を閉じる前に append/write_note の物理着地を必ず ground-truth**（flip 罠と同根 → 下の bs-wiki/bs-docs flip WARNING 参照）: `edit_note` の戻り値が `Created note (append)` でも、実装者 subagent spawn 後は `_blacksheep` に誤着地しうる。canonical（`_sacrificial-lamb` 物理ファイル）の tail を Read で確認し、誤着地なら物理復元 + stray 削除（恒久修正は BM サーバ再起動）。
