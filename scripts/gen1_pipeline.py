"""gen1 パイプライン: build-cache → sweep → oos → post-mortem。

規約 (2026-07-16 グリル確定):
- sweep は 07-09 学習 → 07-13 で 120 セル評価 → 候補 (n≥100, net>0, ratio≥3)
  から net/entry 最大の 1 セルを凍結。候補ゼロなら 07-14 を開けず IS-KILL。
- oos は凍結セルが存在するときのみ、07-09+07-13 で再学習して 07-14 に 1 回だけ
  適用する。artifacts/gen1/oos_lock.json が存在したら再実行を拒否する。
- post-mortem (OOS 全格子) は oos 完了後のみ。そこからの再選択は新 family 扱い。

usage:
  uv run python scripts/gen1_pipeline.py build-cache
  uv run python scripts/gen1_pipeline.py sweep
  uv run python scripts/gen1_pipeline.py oos
  uv run python scripts/gen1_pipeline.py post-mortem
"""
...
def cmd_build_cache() -> None:
    """train/val のみ。OOS 日のラベル・成績は凍結完了前に計算もしない
    (DESIGN.md 実装開始サイン)。07-14 のキャッシュは cmd_oos がロック後に作る。"""
    assert_no_day_leakage()
    for day in [*IS_TRAIN_DAYS, *IS_VAL_DAYS]:
        print(f"building cache for {day}", flush=True)
...
def cmd_oos() -> None:
    ...
    print(f"nulls: {json.dumps({k: {kk: vv for kk, vv in v.items() if kk != 'kind'} for k, v in nulls.items()}, ensure_ascii=False)}",
          flush=True)
...
def main() -> None:
    ...
    if len(sys.argv) != 2 or sys.argv[1] not in cmds:
        raise SystemExit(f"usage: gen1_pipeline.py {{{'|'.join(cmds)}}}")
    cmds[sys.argv[1]]()
